...
export default function Menu() {
  ...
  const col1Order = ['KEBAB', 'BUŁKA', 'DANIE', 'DLA DZIECI'];
  const col1 = displayMenu
    .filter((section) => col1Order.includes(section.category))
    .sort((a, b) => col1Order.indexOf(a.category) - col1Order.indexOf(b.category));

  const col2Order = ['SMASHBURGERY', 'SAŁATKI Z PLANU', 'PLAN NA SEZON'];
  const col2 = displayMenu
    .filter((section) => col2Order.includes(section.category))
    .sort((a, b) => col2Order.indexOf(a.category) - col2Order.indexOf(b.category));
  
  // Napoje zimne, Napoje gorące, Piwa
  const col3 = displayMenu.filter((section) => {
    if (section.category === 'NAPOJE') return true;
    if (section.category === 'PIWA') return true;
    return false;
  });
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-10">
          <div className="flex flex-col justify-between h-full gap-10">
            {col1.map((section, idx) => section._type === 'tableSection' ? renderTableSection(section, idx) : renderListSection(section, idx))}
          </div>
          <div className="flex flex-col justify-between h-full gap-10">
            {col2.map((section, idx) => section._type === 'tableSection' ? renderTableSection(section, idx) : renderListSection(section, idx))}
          </div>
          <div className="flex flex-col justify-between h-full gap-10">
            {col3.map((section, idx) => section._type === 'tableSection' ? renderTableSection(section, idx) : renderListSection(section, idx))}
          </div>
        </div>
      </div>
    </section>
