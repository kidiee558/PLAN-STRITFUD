...
export default function Gallery() {
  ...
  useEffect(() => {
    client.fetch('*[_type == "pageMedia"][0]').then((data) => {
      if (data) {
        if (data.howWeDoItImages?.length > 0) {
          setHowWeDoIt(data.howWeDoItImages.map((img: any) => urlFor(img).width(800).url()));
        }
        if (data.heroGalleryThumbs?.length > 0) {
          setGalleryThumbs(data.heroGalleryThumbs.map((img: any) => urlFor(img).width(600).url()));
        }
        if (data.smileGalleryImages?.length > 0) {
          setSmileImages(data.smileGalleryImages.map((img: any) => urlFor(img).width(600).url()));
        }
      }
    });
  }, []);
  ...
  const displayThumbs = galleryThumbs.length > 0 ? galleryThumbs : fallbackThumbs;
  const displaySmile = smileImages.length > 0 ? smileImages : fallbackSmile;
    ...
    <section id="o-nas" className="py-20 bg-plan-light overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16 max-w-6xl mx-auto">
          {howWeDoIt.length > 0 ? (
            howWeDoIt.slice(0, 3).map((src, idx) => (
              <motion.div 
                key={`how-${idx}`}
            ...
            <>
              <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="w-fit mx-auto cartoon-border bg-plan-orange p-2 md:p-4 transform rotate-1">
                <div className="rounded-xl flex items-center justify-center border-4 border-plan-dark relative overflow-hidden bg-black">
              ...
              <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="w-fit mx-auto cartoon-border bg-plan-orange p-2 md:p-4 transform -rotate-1">
                <div className="rounded-xl flex items-center justify-center border-4 border-plan-dark relative overflow-hidden bg-black">
                  <video className="h-[60vh] md:h-[70vh] w-auto object-cover" autoPlay loop muted playsInline>
              ...
              <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 }} className="w-fit mx-auto cartoon-border bg-plan-orange p-2 md:p-4 transform rotate-2">
                <div className="rounded-xl flex items-center justify-center border-4 border-plan-dark relative overflow-hidden bg-black">
                  <video className="h-[60vh] md:h-[70vh] w-auto object-cover" autoPlay loop muted playsInline>
